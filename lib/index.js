...
Queue.prototype = (function () {
    ...
    var available = function ($this) {
        if ($this.options.allowOverflow && $this.globalQueue.length > 0) {
            var process = findOpenProcess($this);
            if (process != undefined) {
                var request = $this.globalQueue.shift();
    ...
    return {
        ...
        /**
         * Tries to close all of the child processes
         * @param force whether to force close the processes
         */
        close: function(force) {
            utils.each(this.processes, function (index, process) {
                process.close(force);
            });
        },
        /**
